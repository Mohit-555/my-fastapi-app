...
from app.models.models import AssetInventory, AssetMaster, Division, Threshold, Station, Zone, AssetTypeMaster
...
def _build_asset_master_response(record: AssetMaster) -> AssetMasterResponse:
    """Resolve joined fields and build the response schema."""
...
@router.get("/master", response_model=AssetMasterListResponse)
def list_asset_master(
    station_id: Optional[int] = Query(None),
    asset_type_hex: Optional[str] = Query(None),
    is_active: Optional[bool] = Query(None),
    search: Optional[str] = Query(None, description="Search by asset_number_code or smms_asset_code"),
    page: int = Query(1, ge=1),
    page_size: int = Query(50, ge=1, le=500),
    db: Session = Depends(get_db),
):
    """
    List all physical asset instances.
    Filterable by station, asset type, active status, and search term.
    """
    q = db.query(AssetMaster)
    if station_id is not None:
        q = q.filter(AssetMaster.station_id == station_id)
    if asset_type_hex:
        q = q.filter(AssetMaster.asset_type_hex == asset_type_hex.upper())
    if is_active is not None:
        q = q.filter(AssetMaster.is_active == is_active)
    if search:
        term = f"%{search.strip()}%"
        q = q.filter(
            AssetMaster.asset_number_code.ilike(term) |
            AssetMaster.smms_asset_code.ilike(term)
        )
    ...
    records = (
        q.order_by(AssetMaster.station_id, AssetMaster.asset_type_hex, AssetMaster.asset_number_id)
        .offset(offset).limit(page_size).all()
    )
    return AssetMasterListResponse(
        total=total,
        page=page,
        page_size=page_size,
        total_pages=total_pages,
        rows=[_build_asset_master_response(r) for r in records],
    )


@router.get("/master/{asset_id}", response_model=AssetMasterResponse)
def get_asset_master(asset_id: int, db: Session = Depends(get_db)):
    """Get a single physical asset by its internal ID."""
    record = db.query(AssetMaster).filter(AssetMaster.id == asset_id).first()
    if not record:
        raise HTTPException(status_code=404, detail=f"Asset {asset_id} not found")
    return _build_asset_master_response(record)


@router.post("/master", response_model=AssetMasterResponse, status_code=status.HTTP_201_CREATED)
def create_asset_master(payload: AssetMasterCreate, db: Session = Depends(get_db)):
    """
    Register a new physical asset instance.

    asset_number_id must be a 1-byte hex string (00–FF) unique per
    (gateway_id, asset_type_hex) — this is the value used in para_id.
    """
    if payload.asset_type_hex.upper() not in ASSET_TYPE_MAP:
        raise HTTPException(
            status_code=400,
    ...
    gateway = db.query(Gateway).filter(Gateway.id == payload.gateway_id).first()
    if not gateway:
        raise HTTPException(status_code=404, detail=f"Gateway {payload.gateway_id} not found")

    record = AssetMaster(
        smms_asset_code=payload.smms_asset_code,
        smms_asset_name=payload.smms_asset_name,
    ...
    try:
        ...
        raise HTTPException(
            status_code=409,
            detail="An asset with this gateway_id + asset_type_hex + asset_number_id already exists."
        )
    db.refresh(record)
    return _build_asset_master_response(record)


@router.put("/master/{asset_id}", response_model=AssetMasterResponse)
def update_asset_master(
    asset_id: int,
    payload: AssetMasterUpdate,
    db: Session = Depends(get_db),
):
    """Update fields on an existing physical asset."""
    record = db.query(AssetMaster).filter(AssetMaster.id == asset_id).first()
    if not record:
        raise HTTPException(status_code=404, detail=f"Asset {asset_id} not found")
    ...
    try:
        ...
        raise HTTPException(
            status_code=409,
            detail="Conflict: gateway_id + asset_type_hex + asset_number_id must be unique."
        )
    db.refresh(record)
    return _build_asset_master_response(record)


@router.delete("/master/{asset_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_asset_master(asset_id: int, db: Session = Depends(get_db)):
